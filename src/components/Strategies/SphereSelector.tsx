    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            <div className="hidden sm:block">
                ...
                {/* Desktop Dropdown */}
                {isOpen && (
                    <div className={`absolute z-50 top-full right-0 mt-2 w-64 rounded-2xl border shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300 ${
                        theme === 'dark' ? 'bg-[#1a1f26] border-white/10' : 'bg-white border-gray-200'
                    }`}>
            ...
            <div className="sm:hidden">
                ...
                {isMobileMenuOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        />
                        <div className={`w-full max-w-md relative z-10 p-6 rounded-[2.5rem] border ${
                            theme === 'dark' ? 'bg-[#0b1015] border-white/10' : 'bg-white border-gray-100'
                            ...
                            <div className="grid grid-cols-1 gap-3 max-h-[70vh] overflow-y-auto pr-2">
                                {spheres.map((sphere) => (
                                    <button
                                        key={sphere.id}
                                        onClick={() => handleSelect(sphere.id)}
                                        className={`flex items-center gap-4 p-4 rounded-[1.5rem] transition-all relative overflow-hidden ${
                                            activeSphere === sphere.id
                                    ...
                                    >
                                        <div className={`p-2.5 rounded-xl ${
                                            activeSphere === sphere.id ? 'bg-white/20' : theme === 'dark' ? 'bg-white/5' : 'bg-white'
                                        }`}>
                                            {sphere.icon}
                                        </div>
