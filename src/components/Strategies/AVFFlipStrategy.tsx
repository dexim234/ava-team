    ...
    return (
        <div className="space-y-12 animate-fade-in">
            ...
            <div className={`relative overflow-hidden rounded-3xl p-8 border ${theme === 'dark'
                ? 'bg-gradient-to-br from-[#151a21] to-[#0b1015] border-emerald-500/20 shadow-2xl'
                ...
                <div className="relative flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1 space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-400 text-[10px] font-black uppercase tracking-widest border border-emerald-500/20">Scalping</span>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                    { title: "Освоение", desc: "От 1 недели до месяца. Не сравнивайте свой путь с чужим.", icon: <Rocket className="w-5 h-5 text-purple-500" /> },
                    { title: "Потенциал", icon: <TrendingUp className="w-5 h-5 text-blue-500" />, desc: "Возможен выход на $1000/день через месяц практики." },
                    { title: "Тайминг", icon: <Clock className="w-5 h-5 text-amber-500" />, desc: "Сделки 1-5 сек. Важна концентрация, а не глубокий ресерч." }
                ].map((item, idx) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="flex items-center gap-3 mb-2">
                        <LayoutList className={`w-6 h-6 ${theme === 'dark' ? 'text-emerald-400' : 'text-emerald-500'}`} />
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className={`p-4 rounded-xl border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
                                    <h5 className="text-xs font-bold uppercase mb-2 text-emerald-500">Миграции Axiom</h5>
                            ...
                            <div className={`p-4 rounded-xl border border-l-4 border-amber-500/50 ${theme === 'dark' ? 'bg-amber-500/5' : 'bg-amber-50'}`}>
                    ...
                    >
                        <div className={`p-4 rounded-xl border border-l-4 border-emerald-500/50 ${theme === 'dark' ? 'bg-emerald-500/5' : 'bg-emerald-50'}`}>
                            <p className="text-sm font-bold uppercase mb-2 tracking-widest text-emerald-500">Резкая просадка</p>
                            ...
                            <div className="flex flex-col gap-2">
                                {[
                                    ...
                                    <div key={i} className="flex items-center gap-3 text-xs">
                                        <div className="w-5 h-5 rounded-full bg-emerald-500/20 text-emerald-500 flex items-center justify-center font-bold text-[10px]">{i + 1}</div>
                                        {step}
                                    </div>
                                ))}
                            </div>
                            <p className="text-xs text-emerald-500 font-bold mt-2">Важно: Купить на просадке, а не после того, как график уже вернулся на хай.</p>
                        </div>
                    </StrategyStep>
                    ...
                    >
                        <div className={`p-4 rounded-xl border border-dashed ${theme === 'dark' ? 'border-orange-500/30' : 'border-orange-500/20'}`}>
                ...
                <div className="space-y-6">
                    ...
                    <div className={`rounded-2xl p-6 border ${theme === 'dark' ? 'bg-[#151a21]/80 border-white/5' : 'bg-white border-gray-100'
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="space-y-1 pb-2 border-b border-white/5">
                                ...
                                <p className="text-xs text-gray-500">DEV {'<'} 2%. Тор-10 холдеров {'<'} 50%. Ликвидность {'>'} $20k.</p>
                            </div>
                            <div className="space-y-1 pb-2 border-b border-white/5">
                    ...
                    {/* Risk Management */}
                    <div className={`p-6 rounded-2xl border-2 border-dashed ${theme === 'dark' ? 'border-orange-500/30' : 'border-orange-500/20'} space-y-4`}>
                        <div className="flex items-center gap-3">
